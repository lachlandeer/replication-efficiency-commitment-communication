...
rev_mechanism_decisions <-
    ...
    select(date, treatment, session, treatment_simple, num_subjects, 
           group_size, period, 
    ...
    filter(!is.na(value)) %>%
    # for each session since we need to work with the num of subjects in a session to correctly 
    # extract positions in the decisions columns
    group_by(date) %>%
    # filter(date == "151209_1044") %>%
    filter(row_number() %% n_period_length == 0) %>%
    ...
    select(subject_id, period, choice = value)
