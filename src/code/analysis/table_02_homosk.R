...
model_1 <-
    lm(payoff ~ relevel(as.factor(treatment), ref = "Standard Cheap Talk") + 
...
model_2 <-
    lm(min_effort ~ relevel(as.factor(treatment), ref = "Standard Cheap Talk"),
              data = group_level
    )
...
model_3 <-
    lm(num_sevens ~ relevel(as.factor(treatment), ref = "Standard Cheap Talk"),
              data = group_level
    )
...
model_4 <-
    lm(full_coord ~ relevel(as.factor(treatment), ref = "Standard Cheap Talk"),
              data = group_level
    )
...
model_5 <-
    lm(eqn_dev ~ relevel(as.factor(treatment), ref = "Standard Cheap Talk") + 
              sex + game_theory + gpa_imputed,
              data = individual
    )
